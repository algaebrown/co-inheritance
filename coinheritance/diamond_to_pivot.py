...
def initialize(fasta_file, diamond_folder):
    global gid = gene_id(fasta_file)  ### args
    global tid = target_genome_id(diamond_folder)  ### args
...
def concat_diamond_result(diamond_folder, concated_file):
    ...
    for target_genome_blast_result in files:
        ...
        df['target_genome'] = tid[target_genome_blast_result.split("\\")[-1]]  # retain target genome ID
...
import numpy as np
def to_sparse_matrix(smallest_among_all, concated_file):
    """
    ...
    # log e-value wrapper
    def log_e_value_wrapper(e_value):
        return log_e_value(e_value, smallest_among_all)
    ...
    for chunk in chunks:
        ...
        data = data + chunk['e_value'].apply(log_e_value_wrapper())
...
def main(fasta_file, diamond_folder, tmp_folder, bins = 100):
    ...
    smallest_among_all = concat_diamond_result(diamond_folder, tmp_folder + 'concated_diamond')
    ma, data = to_sparse_matrix(tmp_folder + 'concated_diamond', smallest_among_all)
